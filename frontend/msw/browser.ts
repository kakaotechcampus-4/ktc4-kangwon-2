...
export async function ensureControlled() {
  if (typeof navigator === "undefined" || !("serviceWorker" in navigator)) {
    throw new Error("[MSW] 이 브라우저에서는 서비스워커를 사용할 수 없어 개발용 API를 준비하지 못했습니다.");
  }
  if (navigator.serviceWorker.controller) { clearReloadFlag(); return; }
  ...
  if (!readReloadFlag()) {
    ...
    await new Promise(() => { /* 새로고침될 때까지 resolve하지 않는다. */ });
  }

  throw new Error("[MSW] 서비스워커가 이 페이지를 제어하지 못했습니다. 새로고침 후에도 반복되면 DevTools > Application > Service Workers에서 상태를 확인해주세요.");
}
...
const readReloadFlag = () => { try { return sessionStorage.getItem(RELOADED_KEY) === "1"; } catch { return true; } };
const writeReloadFlag = () => { try { sessionStorage.setItem(RELOADED_KEY, "1"); } catch { /* noop */ } };
const clearReloadFlag = () => { try { sessionStorage.removeItem(RELOADED_KEY); } catch { /* noop */ } };
