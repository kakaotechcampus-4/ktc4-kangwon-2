...
const { loadClassSettings, saveClassSettings, totalChildrenFor, CLASS_SETTINGS_CHANGED } = await import("../lib/onboarding/settings.ts");
const { EMPTY_CLASS_SETTINGS, DEFAULT_CHARACTER_MESSAGES } = await import("../lib/onboarding/types.ts");
const { formatDateLabel, isValidDate } = await import("../lib/plan-generator/date.ts");

function storage(value) {
  globalThis.window = Object.assign(new EventTarget(), { sessionStorage: fixtureSession(), localStorage: {
...
test("dashboard count persists and follows saved classroom changes, including zero", () => {
  ...
  let changes = 0;
  window.addEventListener(CLASS_SETTINGS_CHANGED, () => { changes++; });
  const settings = { ...EMPTY_CLASS_SETTINGS, classes: [
    { ...EMPTY_CLASS_SETTINGS.classes[0], currentChildCount: 25 },
    { ...EMPTY_CLASS_SETTINGS.classes[0], id: "class-2", currentChildCount: "", children: [{ id: "a", name: "하늘" }] },
  ] };
...
test("malformed nested values cannot crash classroom and child rendering", () => {
  storage(JSON.stringify({ classes: [null, { className: 1, teacherName: null, ageGroup: "invalid",
    currentChildCount: -2, guardianConsent: "false", children: [null, {}, { name: 42 }, { name: " 하늘 " }] }],
  ...
  assert.deepEqual(classroom.children.map((child) => child.name.slice(-2)), ["하늘"]);
...
test("legacy settings migrate and valid settings round-trip", () => {
  storage(JSON.stringify({ className: "햇살반", ageGroup: "4", children: [{ id: "a", name: "하늘" }] }));
...
test("duplicate IDs are repaired so editing or deleting targets one entry", () => {
  storage(JSON.stringify({ classes: [{ id: "a", children: [{ id: "x", name: "가" }, { id: "x", name: "나" }] }, { id: "a" }] }));
...
test("selected ages round-trip independently, including non-contiguous ages and legacy migration",async()=>{
 ...
 for(const ages of [[3],[3,4],[4,5],[3,5],[3,4,5]]){
   storage(null);
   assert.equal(saveClassSettings({...EMPTY_CLASS_SETTINGS,classes:[{...EMPTY_CLASS_SETTINGS.classes[0],selectedAges:ages,ageGroup:""}]}),true);
 ...
 storage(JSON.stringify({classes:[{ageGroup:"mixed"}]}));assert.deepEqual(loadClassSettings().classes[0].selectedAges,[3,4,5]);
});
test("registered roster wins over reference count while empty roster falls back",()=>{
 for(const [size,expected] of [[0,11],[10,10],[12,12]]){
   const c={...EMPTY_CLASS_SETTINGS.classes[0],currentChildCount:11,children:Array.from({length:size},(_,i)=>({id:String(i),name:"아동"+i}))};
