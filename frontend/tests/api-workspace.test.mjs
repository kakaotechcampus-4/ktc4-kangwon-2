...
async function upload(name,bytes){const body=new FormData();body.append("file",new Blob([bytes]),name);return fetch(`${base}/api/templates/extract`,{method:"POST",body});}
test("template endpoint extracts real TXT and DOCX content",{skip:!base},async()=>{
 const txt=await upload("test.txt","놀이 주제:\n블록 놀이\n교사 지원:\n넓은 블록을 제공한다.");assert.equal(txt.status,200);assert.ok((await txt.json()).headings.includes("교사 지원"));
 const zip=new JSZip();zip.file("[Content_Types].xml",'<?xml version="1.0"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="xml" ContentType="application/xml"/><Override PartName="/word/document.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"/></Types>');zip.file("_rels/.rels",'<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/></Relationships>');zip.file("word/document.xml",'<?xml version="1.0"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>놀이 목표: 아이의 흥미를 관찰합니다.</w:t></w:r></w:p></w:body></w:document>');
 const docx=await upload("test.docx",await zip.generateAsync({type:"nodebuffer"}));assert.equal(docx.status,200);assert.match((await docx.json()).text,/아이의 흥미/);
});
test("template endpoint extracts a text PDF and rejects unsupported or empty files",{skip:!base},async()=>{
 const objects=['<< /Type /Catalog /Pages 2 0 R >>','<< /Type /Pages /Kids [3 0 R] /Count 1 >>','<< /Type /Page /Parent 2 0 R /MediaBox [0 0 300 200] /Resources << /Font << /F1 4 0 R >> >> /Contents 5 0 R >>','<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>'];const stream='BT /F1 12 Tf 20 100 Td (Teacher observation plan test) Tj ET';objects.push(`<< /Length ${stream.length} >>\nstream\n${stream}\nendstream`);let pdf='%PDF-1.4\n';const offsets=[0];objects.forEach((o,i)=>{offsets.push(Buffer.byteLength(pdf));pdf+=`${i+1} 0 obj\n${o}\nendobj\n`;});const xref=Buffer.byteLength(pdf);pdf+=`xref\n0 6\n0000000000 65535 f \n${offsets.slice(1).map(o=>`${String(o).padStart(10,"0")} 00000 n \n`).join("")}trailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n${xref}\n%%EOF`;
 const response=await upload("test.pdf",pdf);const data=await response.json();assert.equal(response.status,200,JSON.stringify(data));assert.match(data.text,/Teacher observation/);
...
test("all added routes render without server failures",{skip:!base},async()=>{
 for(const path of ["records","documents","templates","trends","compare","evaluation","plans/create","plans/setup","plans/start","plans/annual/new","plans/annual/1"]){const response=await fetch(`${base}/${path}`);assert.equal(response.status,200,path);const html=await response.text();assert.ok(!html.includes('id="__next_error__"'),path);}
});
...
async function resolve(path){
 ...
 if(response.status>=300&&response.status<400)return {via:"http",status:response.status,to:response.headers.get("location")};
