...
test("common guard handles unauthenticated access, unfinished onboarding and returning login", async () => {
  ...
  assert.equal(
    settings.saveClassSettings({
      ...EMPTY_CLASS_SETTINGS,
      directorName: "원장",
      regionProvince: "세종특별자치시",
      regionDistrict: "세종특별자치시",
      classes: [
        { ...EMPTY_CLASS_SETTINGS.classes[0], className: "반", teacherName: "담임", ageGroup: "3" },
      ],
      orgName: "새싹어린이집",
...
test("corrupt legacy JSON and malformed legacy objects do not block valid login or signup", async () => {
  ...
  for (const raw of [
    ...
    "[]",
    '{"email":42}',
    '{"email":"old@example.com","salt":null}',
  ]) {
...
test("organization, teacher and onboarding status stay isolated after account switch", async () => {
  ...
  assert.equal(
    settings.planHeaderFor(settings.loadClassSettings(), auth.accountName()),
    "새싹어린이집 · 노덕철 선생님 · 반",
  );
  session.endDemoSession();
  await login("second@example.com");
...
test("legacy accounts missing display names do not substitute classroom teachers for account names", async () => {
  ...
  const account = JSON.parse(values.get(key));
  delete account.name;
  ...
  const saved = {
    ...
    orgName: "새싹어린이집",
    classes: [{ ...EMPTY_CLASS_SETTINGS.classes[0], teacherName: "노덕철" }],
  };
...
test("PRD onboarding URLs, primary class migration, removal and account roles", async () => {
  setup();
  await auth.registerAccount("김수린", "primary@example.com", "test-password-123");
  await login("primary@example.com");
  ...
  settings.saveClassSettings(center);
  assert.equal(auth.onboardingDestination(), "/onboarding/classes");
  const classes = [
    {
  ...
  settings.saveClassSettings({ ...center, classes });
  assert.equal(settings.primaryClassFor(settings.loadClassSettings()).id, "a");
  assert.equal(auth.onboardingDestination(), "/onboarding/children");
  settings.saveClassSettings({ ...center, classes, primaryClassId: "b" });
  assert.equal(settings.primaryClassFor(settings.loadClassSettings()).className, "난초반");
  assert.equal(
    settings.planHeaderFor(settings.loadClassSettings(), auth.accountName()),
    "기관 · 김수린 선생님 · 난초반",
  );
  assert.equal(auth.completeAccountOnboarding(), true);
  assert.equal(auth.loginDestination(), "/");
  settings.saveClassSettings({ ...center, classes: [classes[0]], primaryClassId: "b" });
  assert.equal(settings.primaryClassFor(settings.loadClassSettings()).id, "a");
});
