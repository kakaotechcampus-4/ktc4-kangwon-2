...
registerHooks({resolve(s,c,n){return n(s === "./account-store" ? "./account-store.ts" : s,c);}});
const { hasDemoSession, startDemoSession, endDemoSession, accountStorageKey } = await import("../lib/auth/demo-session.ts");
...
test("active-only, missing, malformed and removed accounts cannot use shared storage", () => {
  ...
  globalThis.window = {localStorage:{getItem:key=>saved.get(key)??null},sessionStorage:{getItem:key=>session.get(key)??null,setItem:(key,v)=>session.set(key,v),removeItem:key=>session.delete(key)}};
...
test("blocked storage and server rendering do not report a successful session", () => {
  globalThis.window = { get sessionStorage() { throw new Error("denied"); } };
  assert.equal(hasDemoSession(), false);
