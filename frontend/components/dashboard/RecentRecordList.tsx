      ...
      <div className="mt-3 rounded-2xl border border-line overflow-hidden">
        ...
        <ul>
          {records.map((r) => (
            <li
              key={r.id}
              className="grid grid-cols-[88px_minmax(0,1fr)_auto] lg:grid-cols-[96px_minmax(0,1fr)_auto] gap-2.5 items-center px-3.5 py-[11px] border-t border-line bg-paper"
            >
              <span className="flex items-center gap-2 min-w-0 text-[13px] font-bold text-ink">
                <span
                  aria-hidden="true"
                  className="inline-flex items-center justify-center w-[26px] h-[26px] rounded-full bg-mint-tint text-mint-ink text-[11px] font-bold shrink-0"
                >
                  {r.childName.slice(-2)}
                </span>
              ...
              <span
                className={`text-[11px] font-medium px-2 py-1 rounded-lg whitespace-nowrap ${BADGE[r.status]}`}
              >
