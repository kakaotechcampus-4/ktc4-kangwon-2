...
import { CLASS_SETTINGS_CHANGED, loadClassSettings, totalChildrenFor } from "@/lib/onboarding/settings";
...
export default function HomePage() {
  ...
  const tasks = TODAY_TASKS.map(task=>task.id==="records"?{...task,count:data.observations.length,description:"교사가 남긴 실제 관찰 기록을 확인해 주세요.",href:"/records"}:task.id==="drafts"?{...task,count:data.documents.filter(d=>d.status==="draft").length,href:"/documents"}:{...task,title:"평가제·증빙 점검",description:"문서의 항목과 증빙 연결을 확인해 주세요.",count:data.documents.filter(d=>d.status==="confirmed").length,href:"/evaluation",cta:"증빙 점검"});
  const recentRecords=data.observations.slice(0,4).map(r=>({id:r.id,childName:r.childName,text:r.fact,status:"review" as const}));
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_352px] xl:grid-cols-[minmax(0,1fr)_352px] min-h-[calc(100vh-110px)]">
          ...
          <div>
            ...
            <div className="flex items-baseline justify-between gap-3 mt-1">
              <h2 className="font-display text-[20px] text-ink">
                오늘의 기록과 문서<span className="font-mono text-[14px] text-ink-soft ml-1.5">{tasks.reduce((n, t) => n + t.count, 0)}</span>
              </h2>
              <Link href="/documents" className="text-[13px] text-ink-soft hover:text-sage-ink">문서 보기 →</Link>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {tasks.map((t) => <DashboardTaskCard key={t.id} task={t} />)}
          </div>
          {error&&<p role="alert" className={ws.error}>{error}</p>}
          <section className={ws.hero}><div><div className={ws.eyebrow}>쓱싹요정 · 선생님의 하루를 잇다</div><h2>관찰에서 계획으로,<br/>기록에서 다음 지원으로.</h2><p>우리 원 양식과 새로운 놀이 아이디어를 연결해보세요.</p></div></section>
          <div className={ws.cards}>{[["/templates","▤","원 양식 분석","기존 문서의 구조와 문체를 계획안에 활용해요."],["/plans/annual/new","▦","연간·월간·주간 계획안","우리 반에 맞는 놀이 흐름을 준비해요."],["/trends","✳","트렌드봇","공식 교육 자료에서 다음 놀이의 힌트를 찾아요."],["/compare","↗","기록 간 비교","아동의 이전과 현재 기록을 나란히 살펴봐요."]].map(([href,icon,title,description])=><Link key={href} href={href} className={ws.card}><span className={ws.badge}>{icon}</span><h3 style={{marginTop:15}}>{title}</h3><p className={ws.muted}>{description}</p><span className={ws.source}>시작하기 →</span></Link>)}</div>
        </section>
        ...
        <aside className="min-w-0 flex flex-col gap-[22px] bg-paper border-t lg:border-t-0 lg:border-l border-line px-4 pt-5 pb-7 lg:px-[26px] lg:pt-[26px] lg:pb-10">
          <div className="order-1">{totalChildren === null
            ? <p role="status" className={ws.hint}>원아 수를 불러오는 중이에요.</p>
            : <AttendanceSummary data={{ total: totalChildren, present: totalChildren, absent: 0 }} />}</div>
          ...
          <div className="order-2 lg:order-5"><RecentRecordList records={recentRecords} />{!recentRecords.length&&<p className={ws.hint}>첫 관찰 기록을 남기면 여기에 표시돼요.</p>}</div>
        </aside>
      </div>
    </>
  );
