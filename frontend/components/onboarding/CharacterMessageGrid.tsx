...
export default function CharacterMessageGrid({
  ...
  const editedCount = MONTH_ORDER.filter(
    (m) => messages[m] !== DEFAULT_CHARACTER_MESSAGES[m],
  ).length;

  return (
    <div
      className={`flex flex-col gap-3 ${disabled ? "opacity-55" : ""}`}
    ...
    >
      <div className="flex flex-col items-start gap-0.5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <TextButton
          onClick={onResetAll}
          disabled={disabled || editedCount === 0}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {!disabled && edited && (
          <span className="font-mono text-[10px] px-1.5 rounded-full bg-peach-tint text-peach-ink">
            수정됨
          </span>
        )}
      </div>
