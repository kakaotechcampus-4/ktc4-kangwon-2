...
```json
{ "error": { "code": "VALIDATION_FAILED", "message": "사람이 읽는 문장", "field": "age_min" } }
```
...
```json
{ "name": "햇님반", "age_min": 3, "age_max": 4, "child_count": 18, "teacher_name": "김선생" }
```

`age_min` ≤ `age_max`, 둘 다 3~5. `child_count` 는 **선택**(null 허용).

**FE 는 드롭다운 하나만 보여주고 내부에서 두 값으로 바꿔 보낸다.**

```
만 3세          → 3 · 3        만 3~4세 (혼합)  → 3 · 4
만 4세          → 4 · 4        만 4~5세 (혼합)  → 4 · 5
만 5세          → 5 · 5        만 3~5세 (혼합)  → 3 · 5
```
