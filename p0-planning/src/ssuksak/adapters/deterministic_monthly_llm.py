...
class DeterministicMonthlyLlm:
    ...
    monthly_requests: list[MonthlyPlanningRequest] = field(
        default_factory=list
    )
    cell_requests: list[MonthlyCellPlanningRequest] = field(
        default_factory=list
    )

    def generate_monthly(
        self, request: MonthlyPlanningRequest
    ) -> RawLlmResponse:
        self.monthly_requests.append(request)
        return RawLlmResponse(self.monthly_content, self.model, "fake-monthly-1")

    def generate_cell(
        self, request: MonthlyCellPlanningRequest
    ) -> RawLlmResponse:
